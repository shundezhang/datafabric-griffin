...
//import au.org.arcs.griffin.filesystem.impl.jargon.JargonFileSystemImpl;
...
/**
 * SFTP to Irods server details. 
 * Includes the options and backend griffin fileSystem
 * 
 * @author John Curtis
 */
public class SftpServerDetails implements SftpServerConstants
{
	...
	public SftpServerDetails(String beanPath,String logDetailsPath)
	{
		...
		{
			System.err.println("Unable to load app resources: "+e.getMessage());
			e.printStackTrace();
			System.exit(1);   
	 	}
		...
		if(beanPath==null || beanPath.isEmpty())
			this.beanPath="./"+DEFAULT_BEAN_RES;//look in jar directory for context.xml
		this.logDetailsPath = logDetailsPath;
		if(logDetailsPath==null || logDetailsPath.isEmpty())
			this.logDetailsPath="./"+DEFAULT_LOG_RES;//look in jar directory for log4j.properties
	}

	public SftpServerDetails(String[] args)
	{
		this(	args.length > 0 ? args[0] : "",
	 ...
	 */
	public SftpSessionContext createSftpContext(String user, FileSystemConnection connection,ResourceBundle res_bundle)
	{
	...
	private ApplicationContext loadApplicationContext(String beanResFile)
	{
		...
		{
			appContext = new FileSystemXmlApplicationContext("file:/opt/griffin/" + beanResFile);
		}
		/*
		else	//Use res version
		{
			...
			appContext = new ClassPathXmlApplicationContext(new String[]{ "/" + DEFAULT_BEAN_RES });
		}
		*/
		return appContext;
	}
	...
	//  2. options.getProperty(OPT_ROOT_DIR) in the context file if above not already set
	//  3. Parent directory of the context file if OPT_ROOT_DIR not added to context file
...
//		System.setProperty(FileSystem.GRIFFIN_HOME_DIR, dir.getAbsolutePath());
		return true;
	}
	...
	//  2. options.getProperty(OPT_LOG_DIR) in the context file if above not already set
	 ...
	 */
	/*
	private void initLoggingDir() throws IOException
	{
		if (getSystemLoggingDirectory() != null)
			return;	//Done
		
		//Check for explicit directory in options
		String opt_log_dir=options.getLoggingDirectory();
		if(setLoggingDir(opt_log_dir))
			return;
		//Fail if we had one but unable to set
		if(opt_log_dir!=null && opt_log_dir.length()>0)
			throw new IOException("Unable to set the context log directory: "+opt_log_dir);	
		
		//Use the system dir
		String home_dir=getSystemHomeDirectory();
		if (home_dir == null || home_dir.isEmpty())
			throw new IOException("Expected a system home directory to be set");	
		
		File log_dir=new File(home_dir,"logs");
		log_dir.mkdir();
		if(!log_dir.exists() || !log_dir.isDirectory())
			throw new IOException("Failed to create log directory: "+log_dir.getAbsolutePath());
		if(setLoggingDir(log_dir.getAbsolutePath()))
			return;
...
*/
	 ...
	 */
	/*
	private boolean setLoggingDir(String dir_path)
	{
		if(dir_path==null)
			return false;
		...
		System.setProperty(SYS_LOG_DIR, dir.getAbsolutePath());
		return true;
	}
*/
	/**
	...
	public int getOptimalFilePacketBytes()
	{
		...
		if(ideal_packet_bytes>MIN_FILE_TRANSFER_SIZE && ideal_packet_bytes<=MAX_FILE_TRANSFER_SIZE)
