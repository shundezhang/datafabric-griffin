...
//                            return;
    ...
    protected void readFile(SshFile path) throws IOException {
        if (log.isDebugEnabled()) {
            log.debug("Reading file {} using buffer size ", path, getBufferSize(path.getSize()));
        }
        ...
        try {
            byte[] buffer = new byte[getBufferSize(path.getSize())];
            for (;;) {
    ...
    private int getBufferSize(long fileSize) {
		...
		if (bufferSize < 51200)
			bufferSize = 51200;
		// maximum buf size of 10MegaByte
		else if (bufferSize > 15728640) //10485760) //5242880)
			bufferSize = 15728640;
